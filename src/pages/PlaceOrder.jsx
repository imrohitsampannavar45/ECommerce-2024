...
const PlaceOrder = () => {
  ...
  return (
    <div className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      {/* Right Side */}
      <div className='mt-8 flex flex-col items-end w-full sm:max-w-[480px]'>
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            <div onClick={() => setMethod('stripe')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <div className={`w-5 h-5 flex items-center justify-center border rounded-full ${method === 'stripe' ? 'bg-green-400' : ''}`}>
                {method === 'stripe' && <div className='w-2.5 h-2.5 bg-white rounded-full'></div>}
              </div>
              <img className='h-5 mx-4' src={assets.stripe_logo} alt="Stripe" />
            </div>

            <div onClick={() => setMethod('razorpay')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <div className={`w-5 h-5 flex items-center justify-center border rounded-full ${method === 'razorpay' ? 'bg-green-400' : ''}`}>
                {method === 'razorpay' && <div className='w-2.5 h-2.5 bg-white rounded-full'></div>}
              </div>
              <img className='h-5 mx-4' src={assets.razorpay_logo} alt="Razorpay" />
            </div>

            <div onClick={() => setMethod('cod')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <div className={`w-5 h-5 flex items-center justify-center border rounded-full ${method === 'cod' ? 'bg-green-400' : ''}`}>
                {method === 'cod' && <div className='w-2.5 h-2.5 bg-white rounded-full'></div>}
              </div>
